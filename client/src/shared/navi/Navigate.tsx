...
const Navigate = () => {
  ...
  const { isBackHome, isReload, isHide } = useSelector(
    (state: RootState) => state.navicom
  );
  ...
  const toggleSearchBox = (isClose?: boolean) => {
    ...
    } else setIsShowSearchBox(!isShowSearchBox);
  };
  // ---------------------------------------
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/categories`, { timeout: 5000 }) // Timeout 5s
      .then((res: any) => {
        if (res.data) {
  ...
  useEffect(() => {
    if (searchValue !== "") {
      SetIsSearching(true);
      axios
        .get(`${API_URL}/products?keyword=${searchValue}`)
        .then((res: any) => {
          if (res.data) {
  ...
  if (isHide) {
    ...
    return (
      <div className="position-fixed z-2 bottom-0 w-100 d-flex justify-content-center align-items-center p-4 gap-5">
        <div onClick={() => navigate("")} className="navi-item-home">
          Trang chủ
        </div>
      </div>
    );
    ...
    return (
      <div className="navi-full-container position-fixed z-3 bottom-0 w-100 d-flex justify-content-center align-items-center p-2 gap-5 p-sm-3">
        <div className="navi-item z-2 home" onClick={() => naviHandler("")}>
              ...
              <div className="result-container flex-grow-1 p-4">
                  ...
                  <div className="categories h-100 w-100 d-flex flex-column">
                    ...
                    <div className="cate-list-container flex-grow-1 d-flex flex-row py-3">
                      <div className="cate-options d-flex flex-column gap-4">
                        <div
                          className={`cate-option-item d-flex justify-content-start align-items-center gap-2 py-3 ${
                            crrGender === "Nữ" ? "active" : ""
                          }`}
                          onClick={() => {
                            SetCrrGender("Nữ");
                        ...
                        {categoriesShow.map((cate: any, index: number) => {
                          return (
                            <div
                              onClick={() => {
                                naviHandler(
                                  `${crrGender}/${normalizeStringToPath(
                                    cate.name
                                  )}`,
                                  cate._id
                                );
                ...
                {isSearching && (
                  <div className="result-container d-flex flex-column gap-3 overflow-auto">
                    {delay && (
                      <div>
                        <i className="pi pi-spin pi-spinner"></i> &nbsp; Đang
                        tìm kiếm sản phẩm...
                      </div>
                    )}
                    {items.length > 0 ? (
