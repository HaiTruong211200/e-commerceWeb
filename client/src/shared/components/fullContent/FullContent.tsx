...
const FullContent = ({
  ...
  const [imageURL, setImgaeURL] = useState(
    image || "images/fullcontent-default.jpg"
  );
  ...
  return (
    <div className="content-full-container position-relative w-100 h-100 d-flex justify-content-center align-items-center">
      ...
      />
      <div className="position-relative z-2 content-container h-100 d-flex flex-column justify-content-center pb-3 ">
        <div
          className="content__title fw-semibold"
          onClick={() => {
            if (cateLabel?.id)
              naviHandler(
                `/${cateLabel.crrGender}/${normalizeStringToPath(
                  cateLabel.name
                )}`,
                cateLabel.id
              );
          ...
          style={{ cursor: link ? "pointer" : "default", color: "white" }} // đổi màu nếu muốn
        >
          {title?.toUpperCase()}
        </div>
        <div className="content__des fw-semibold">{des}</div>
        {price && <div className="content__price">{price}</div>}
        {/* <div className="content__link">{link}</div> */}
        {loading && (
          <div className="content__loading h-25 mt-5 fs-2 text-white fw-bolder">
            <i className="fs-2 pi pi-spin pi-spinner"></i> &nbsp; Đang tải...
          </div>
        )}
        {productList && productList.length > 0 ? (
          <div className="position-relative">
            ...
            {cateLabel && (
              <span
                className="list-label label-ad text-white px-2 fs-3 rounded-3 shadow-sm"
                onClick={() => {
                  if (cateLabel.id)
                    naviHandler(
                      `/${cateLabel.crrGender}/${normalizeStringToPath(
              ...
              >
                {String(cateLabel.name).toUpperCase()}{" "}
                <i className="pi pi-link"></i>
              </span>
            )}
            <div
              ref={scrollRef}
              className="content__product-list gap-1 no-scrollbar "
            >
              {productList.length > 4 && (
                <div className="scroll-buttons w-100 d-none d-md-flex justify-content-between align-items-center position-absolute top-50 mt-5 start-0 end-0 translate-middle-y px-2">
                  <button
                    className="btn btn-outline-dark rounded-circle shadow-lg"
                    onClick={() => scroll("left")}
                  >
